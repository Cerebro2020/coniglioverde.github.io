...
export default function(choose,quadri){
  ...
  // DEER
   // CERVO
   const loaderDeer = new OBJLoader();
   let deer;
   // LOAD A RESOURCE
   loaderDeer.load('3d/deer.obj',
   function ( object ) {
       object.position.set( 160, 0.51,-80 );      
       object.rotation.set( 0, -Math.PI/2, 0 );      
       try{
       const matDeer=new THREE.MeshPhysicalMaterial({
         color: 0XFF00B4,
         roughness:0.0,
         metalness:0.5,                    
         }) 
       object.children[0].material=matDeer;
       }catch(e){
       console.log(e);
       }
       object.castShadow = true;
       deer=object;     
       console.log( 'body was loaded', deer );
       scene.add( deer );      
       deer.scale.set( 1, 1, 1);  
       //DEER 2       
       let deer2 = deer.clone();
       deer2.position.set( -90, 0.51, 120 );
       deer2.rotation.set( 0, 5, 0 );     
       scene.add( deer2 );
       deer2.castShadow = true; 
       deer2.receiveShadow = true; 
       //DEER 3       
       let deer3 = deer.clone();
       deer3.position.set( -210, 80, 0 );
       deer3.rotation.set( -1, Math.PI/0.5, -Math.PI/2 );     
       scene.add( deer3 );
       deer3.castShadow = true; 
       deer3.receiveShadow = true; 

        //DEER 4       
        let deer4 = deer.clone();
        deer4.position.set( -210, 85, -20 );
        deer4.rotation.set( -0.5, Math.PI/0.5, -Math.PI/2 );     
        scene.add( deer4 );
        deer4.castShadow = true; 
        deer4.receiveShadow = true; 
     },
     // called when loading is in progresses
     function ( xhr ) {
       console.log( ( xhr.loaded / xhr.total * 100 ) + '% loaded' );
